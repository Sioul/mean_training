...
describe('Category Model', function() {
	...
	it('throws validation error when name is empty', function(done) {
		...
		should.exist(err);
		err.errors.name.message.should.equals('name must be 15 chars in length or less');
		done();
	    });
	});
	...
	it('throws validation error when name longer than 15 chars', function(done) {
	    ...
	    category.save(function(err) {
		should.exist(err);
		err.errors.name.messsage.should.equal('name must be 15 chars in length or less');
		done();
	    });
	});
